...
class Attack(object):
    ...
    def _create_attack(self, attack):
        ...
        if attack == "textfooler":
            ...
            transformation = WordSwapMaskedLM(method="bert-attack", max_candidates=2)
            constraints = [RepeatModification(), StopwordModification()]
...
class AdvPromptAttack:
    ...
    def get_indices_to_order(self, current_text, **kwargs):
        ...
        Returns:
            The length and a consistently selected subset of indices which search methods can use to search/order.
        """
        ...
        #---------------按序号选择特定元素形成新的indices_list-------------#
        # Convert indices_to_order to list
        indices_list = list(indices_to_order)
        # 按序号选择特定元素形成新的indices_list
        selected_indices = [2, 3, 6]  # 使用序号选择
        ordered_indices_to_order = [indices_list[i] for i in selected_indices if i < len(indices_list)]
        return len(ordered_indices_to_order), ordered_indices_to_order
        #----------------------------------------------------------------#
    ...
    def _attack(self, initial_result):
        ...
        if result.original_result.output == 0:
            pdr = float('inf')  # 或者选择一个合适的默认值
        else:
