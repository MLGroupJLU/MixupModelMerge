...
class MergedModel_Math(LMMBaseModel):
    """
    ...
    def __init__(self, ckpt_path, max_new_tokens, temperature, device):
        # super(MergedModel_Math, self).__init__(ckpt_path, max_new_tokens, temperature, device)
        # self.model = None 
        # self.tokenizer = None
        # self.max_new_tokens = max_new_tokens
        # self.temperature = temperature
        ...
        # # 如果 device 是 "0,1" 这样的字符串，则转换为适合的 device_map
        # if isinstance(device, str) and "," in device:
        #     devices = device.split(',')
        #     self.device_map = {i: f"cuda:{d.strip()}" for i, d in enumerate(devices)}
        # elif device == "auto":
        #     self.device_map = "auto"  # 自动分配所有可用 GPU
        # else:
        #     self.device_map = {0: f"cuda:{device}"}
            
        # # 从指定路径加载模型
        # self.load_model(ckpt_path)
        self.model = create_llm(finetuned_model_name=ckpt_path, pretrained_model_name=None,
                         args=None, logger=None, tensor_parallel_size=2,
    ...
    def predict(self, input_text, **kwargs):
        ...
        completions = self.model.generate(input_text, sampling_params)
