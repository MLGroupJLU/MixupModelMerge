...
def create_llm(finetuned_model_name, pretrained_model_name, args, logger: logging.Logger, tensor_parallel_size=1, just_inference=False, save_model_path=None):
    ...
    if not os.path.exists(finetuned_model_name):
        logger.error(f"模型路径 {finetuned_model_name} 不存在。")
        raise FileNotFoundError(f"模型路径 {finetuned_model_name} 不存在。")
    
    # 检查 save_model_path 是否为 None
    if save_model_path is not None:
        logger.warning("save_model_path 应该为 None。")
