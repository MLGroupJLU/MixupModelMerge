...
#评估merge后的模型
    ...
    for save_model_path in save_model_paths:
        if save_model_path is not None:
            shutil.rmtree(save_model_path, ignore_errors=True) #删除保存模型的文件夹（如果存在），以便在保存新的模型之前清理旧的文件
...
parser.add_argument("--tensor_parallel_size", type=int, default=2, help="numbers of gpus to use")
parser.add_argument("--param_value_mask_rate", type=float, default=0.5, help="ties: mask rate of the smallest-magnitude parameter values")
parser.add_argument("--mixup", action="store_true", default=False, help="whether to use mixup model merge")
parser.add_argument("--alpha", type=float, default=0.5, help="Key parameters of the mixup model merge method")
...
if __name__ == "__main__":
    ...
    for merge_flag, task_name in zip([args.merge_instruct, args.merge_math, args.merge_code], ["instruct", "math", "code"]):
        if merge_flag:
            finetuned_model_names.append(task_model_mapping_dict[task_name]) #["WizardLM-13B-V1.2", "WizardMath-13B-V1.0"]
    ...
    args.weight_mask_rates = [args.weight_mask_rate for _ in range(len(finetuned_model_names))] #[0.2, 0.2] #基于 args.weight_mask_rate 的值为每个 finetuned_model_names 列表中的模型创建一个相同的 weight_mask_rate 列表

    if args.merging_method_name == "ties_merging":
        args.save_model_name = f"{args.merging_method_name}"
    elif args.merging_method_name == "average_merging":
        args.save_model_name = f"{args.merging_method_name}"
        ...
        if args.mask_apply_method == "ties_merging":
    ...
    for finetuned_model_name in finetuned_model_names: #["WizardLM-13B-V1.2", "WizardMath-13B-V1.0"]
